...
function makeIsolatedEnvironment({ runtimeRoot, dataRoot }) {
  // Deliberately do not inherit the host PATH, PYTHONPATH, virtualenv or conda
  // settings. A few OS-level variables are preserved because the operating
  // system itself needs them (for example SystemRoot on Windows).
...
async function ensureRuntimeDirectories(env) {
  const dirs = [
    env.HOME,
    env.APPDATA,
    env.LOCALAPPDATA,
...
function pythonString(value) {
  // JSON string syntax is valid Python string syntax for the characters used in
  // paths and random tokens, including escaped Windows backslashes.
...
class JupyterManager {
  constructor({ runtimeRoot, dataRoot, workspaceRoot, log = console }) {
  ...
  async start() {
    ...
    await ensureRuntimeDirectories(this.env);
  ...
  _logJupyter(chunk) {
    // Avoid leaking the random token into logs.
  ...
  async saveAll() {
    ...
    if (!ready) throw new Error('JupyterLab save interface is not ready');
    const result = await this.view.webContents.executeJavaScript(
      `(async () => {
        await window.jupyterapp.commands.execute('docmanager:save-all');
        return true;
      })()`,
      true
    );
    if (result !== true) throw new Error('JupyterLab did not confirm save-all');
  }
